...
contract AddressLookup is Ownable {
    ...
    function batchNamesAndAvatars(address[] calldata _addr) external view returns(Account[] memory) {
        ...
        for(uint256 i; i < _addr.length; i++ ){
            ...
            bytes32 reverse_node = reverse_registrar.node(account.addr);
            account.name = reverse_resolver.name(reverse_node);
            bytes32 node = getDomainHash(account.name);
    ...
    function batchAccountsFull(address[] calldata _addr) external view returns(AccountFull[] memory) {
    ...
    function batchMetaData(tokenInfo[] calldata _tokens) external view returns(metadataInfo[] memory) {
        metadataInfo[] memory _metadata = new metadataInfo[](_tokens.length);

        for (uint i = 0; i < _tokens.length; i++) {
            if (_tokens[i].scheme == 721) {
                string memory metadataURI = IERC721Metadata(_tokens[i].token).tokenURI(_tokens[i].tokenId);
                _metadata[i] = metadataInfo(_tokens[i].token, _tokens[i].tokenId, metadataURI);
            }

            if (_tokens[i].scheme == 1155) {
                string memory metadataURI = IERC1155MetadataURI(_tokens[i].token).uri(_tokens[i].tokenId);
                _metadata[i] = metadataInfo(_tokens[i].token, _tokens[i].tokenId, metadataURI);
            }
        }
        return _metadata;
    ...
    function getParts(string memory _string) internal view returns(string[] memory) {
    ...
    function getDomainHash(string memory _ensName) internal view returns (bytes32 namehash) {
